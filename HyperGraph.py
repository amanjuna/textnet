...
class HyperGraph:
    def __init__(self, node_vectors):
        self.partition = {}
        embs = []
        node_names = []
        for item in node_vectors:
            node_names.append(item[0] + ' ' + item[1])
            embs.append(item[2])
        embs = np.array(embs)
        d = embs.dot(embs.T)
    ...
    def get_c(self, trials=1000):
        return nx.average_clustering(self.graph, trials)
...
if __name__=="__main__":
    ...
    for i in range(num_nodes):
        vecs.append(('name' + str(i), 'file' + str(i), np.random.random_sample((5, ))))
    #print(vecs)
    G = HyperGraph(vecs)
